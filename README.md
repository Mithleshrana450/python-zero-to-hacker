# Python Zero to Hacker 🐍

A structured 12-month Python learning journey from complete beginner
to ethical hacker and cybersecurity expert.

## Roadmap
- ✅ Phase 1 — Python Fundamentals (in progress)
- ⏳ Phase 2 — Logic Building
- ⏳ Phase 3 — Core Programming
- ⏳ Phase 4 — Data Structures
- ⏳ Phase 5 — Intermediate Python
- ⏳ Phase 6 — Object Oriented Programming
- ⏳ Phase 7 — Advanced Python
- ⏳ Phase 8 — Databases
- ⏳ Phase 9 — API and Web Development
- ⏳ Phase 10 — Automation
- ⏳ Phase 11 — Ethical Hacking with Python
- ⏳ Phase 12 — Security Programming
- ⏳ Phase 13 — Job Ready Preparation

## Daily Progress
| Day | Topic | Status |
|-----|-------|--------|
| Day 1 | Variables, Data Types, Input/Output | ✅ |
| Day 2 | Operators, Type Casting, Truthiness | ✅ |
| Day 3 | If/Else, Nested Conditions, Logic Building | ✅ |
| Day 4 | Loops — for, while, break, continue | ✅ |

## Goal
Become a Python Master + Ethical Hacker 🎯

## Student
Mithlesh Rana — BCA Cyber Security, Parul University